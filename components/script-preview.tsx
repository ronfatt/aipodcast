  ...
  return (
    <div className="space-y-4">
      {turns.map((turn, index) => {
        const showSegmentLabel = index === 0 || turns[index - 1]?.segment !== turn.segment;

        return (
          <div key={turn.id}>
            {showSegmentLabel && turn.segment ? (
              <p className="mb-2 text-xs uppercase tracking-[0.3em] text-coral">
                {turn.segment.replace(/_/g, " ")}
              </p>
            ) : null}
            <div
